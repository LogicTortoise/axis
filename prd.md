...
*   **产品名称:** Axis
*   **产品定位:** Axis是一款专为个人开发者设计的单用户Web端项目管理工具，旨在提供高效、自动化的多工作区待办事项管理能力，通过后端API实现任务的下发与状态追踪，并支持灵活的任务队列和自定义钩子（Hooks），以提升开发者的项目管理效率和自动化水平。本工具使用SQLite数据库存储数据，适合个人本地使用和轻量化部署。
...
*   **后端API驱动的任务自动化:** 通过后端API与外部系统对接，实现任务下发执行和状态追踪的自动化。
*   **实时任务状态追踪:** 通过轮询数据库和外部系统，实时更新任务状态，确保项目进度透明。
*   **高效任务队列与钩子:** 支持任务队列批量提交、顺序执行，并允许通过curl命令自定义任务前置/后置Hook操作，实现高度定制化工作流。
*   **即时通知与异常处理:** 任务完成或失败时生成通知，并支持失败任务的快速重试，提升异常处理效率。
*   **轻量化本地存储:** 使用SQLite数据库，数据本地存储，易于备份和迁移，无需额外数据库服务。
...
| F-WKS_004 | 工作区元信息管理 | 提供独立的界面用于管理工作区的元信息，包括Hooks配置（任务开始/结束钩子）、详细项目描述等。 | P0 |
...
| F-TASK_003 | 查看任务列表 | 在工作区详情页展示任务列表，包含任务名称、当前状态、优先级、任务来源（手动创建）、创建/更新时间等关键信息。 | P0 |
| F-TASK_004 | 任务状态更新（手动/自动） | 用户可手动更新任务状态（如待执行、进行中、已完成、失败）。同时，系统根据后端执行结果自动更新任务状态。 | P0 |
| F-TASK_005 | 轮询机制追踪任务状态 | 对于已下发的任务，后端通过轮询机制定期向执行系统查询任务的最新状态，并同步更新到Axis中。 | P0 |
| F-TASK_006 | API下发任务 | 支持将Axis中的任务通过后端API下发到指定执行系统，实现任务的自动化执行。 | P0 |
...
| F-QUEUE_005 | 配置Start Hook | 支持为单个任务配置Start Hook，即任务开始执行前自动执行一个curl命令（用户提供完整的curl命令文本）。 | P0 |
| F-QUEUE_006 | 配置Stop Hook | 支持为单个任务配置Stop Hook，即任务执行完成后（无论成功或失败）自动执行一个curl命令（用户提供完整的curl命令文本）。 | P0 |
...
graph TD
    ...
    subgraph "数据存储层"
        M["SQLite数据库 (Workspaces, Tasks, Hooks, Queues, Notifications)"]
        N["外部执行系统 (下发/执行任务)"]
    end
...
| **创建任务**   | 手动创建待办事项                         | 便捷、直观                        | 任务创建流程是否简单高效               | **P-WKS_DETAIL (工作区详情)** 添加任务，**F-TASK_001 (手动添加任务)** |
...
```mermaid
graph TD
    A["用户手动创建任务 (P-WKS_DETAIL)"] --> B["任务进入工作区列表 (P-WKS_DETAIL)"]
    B --> C{"是否需要下发执行?"}
    C -- "是" --> D["用户选择任务，下发到队列或直接下发 (P-WKS_DETAIL/P-QUEUE_MANAGE)"]
    D --> E["后端触发Start Hook (如果配置) (F-QUEUE_005)"]
    E --> F["执行curl命令调用外部服务"]
    F --> G["Axis通过API下发任务 (F-TASK_006)"]
    G --> H["外部执行系统接收并执行任务"]
    H --> I["后端轮询获取任务执行状态 (F-TASK_005)"]
    I --> J{"任务执行完成/失败?"}
    J -- "完成" --> K["更新任务状态为'已完成' (F-TASK_004)"]
    J -- "失败" --> L["更新任务状态为'失败' (F-TASK_004)"]
    K --> M["后端触发Stop Hook (如果配置) (F-QUEUE_006)"]
    L --> M
    M --> N["执行curl命令调用外部服务"]
    N --> O["生成任务完成/失败通知 (F-NOTIF_001)"]
    O --> P["用户在通知中心查看 (P-NOTIF_CENTER)"]
    J -- "否" --> I
```
...
graph TD
    ...
    E --> F["队列开始执行第一个任务"]
    F --> G["后端触发当前任务的Start Hook (F-QUEUE_005)"]
    G --> H["执行curl命令"]
    H --> I["下发任务到执行系统 (F-TASK_006)"]
    I --> J["后端轮询追踪任务状态 (F-TASK_005)"]
    J --> K{"任务完成/失败?"}
    K -- "完成" --> L["更新任务状态 (F-TASK_004)"]
    K -- "失败" --> M["更新任务状态 (F-TASK_004)"]
    L --> N["触发Stop Hook (F-QUEUE_006)"]
    M --> N
    N --> O["执行curl命令"]
    O --> P{"队列中还有待执行任务?"}
    P -- "是" --> Q["执行下一个任务 (F-QUEUE_003)"]
    Q --> G
    P -- "否" --> R["队列执行完毕，生成队列完成通知 (F-NOTIF_001)"]
    M --> S["用户在P-QUEUE_MANAGE查看失败原因 (F-QUEUE_004)"]
    S --> T{"用户重试任务?"}
    T -- "是" --> G
    T -- "否" --> R
```
...
4.  **下发任务到执行系统**
    *   进入特定**工作区详情 (P-WKS_DETAIL)**。
    *   在任务列表中，点击任务的"下发"按钮，将任务通过后端API下发到外部执行系统。
    *   系统通过轮询机制自动追踪任务执行状态。
5.  **创建和管理任务队列**
...
*   **任务 (Task):**
    ...
    *   `queue_status` (none, queue, running)
    *   `api_data` (TEXT/JSON)
    *   `execution_id`
    *   `dispatch_time`
    *   `start_hook_curl` (完整curl命令文本)
    *   `stop_hook_curl` (完整curl命令文本)
    *   `created_at`
    *   `updated_at`
*   **钩子配置 (HookConfig):**
    *   `id` (PK)
    *   `task_id` (FK to Task, 可空，表示工作区级钩子)
    *   `workspace_id` (FK to Workspace, 可空，表示任务级钩子)
    *   `name`
    *   `type` (Start, Stop)
    *   `curl_command` (完整curl命令文本)
    *   `trigger_condition` (always, success, failure)
    *   `enabled` (boolean)
    *   `last_execution`
    *   `created_at`
    *   `updated_at`
...
*   **通知 (Notification):**
    ...
    *   `is_read` (boolean)
    *   `action_data` (TEXT/JSON, 如重试任务的参数)
    *   `created_at`

**技术栈说明：**
*   数据库：SQLite 3，单文件存储，适合个人使用
*   后端任务调度：通过定时任务轮询数据库表实现
*   Hooks执行：后端通过执行存储的curl命令实现
*   通知机制：后端定时轮询通知表，前端定期拉取
...
graph TD
    ...
    subgraph "Axis后端服务"
        ...
        BE_C["任务调度服务（轮询）"]
        BE_D["通知服务（轮询）"]
        BE_E["Hooks执行器（curl命令）"]
    end

    subgraph "外部服务"
        EXT_EXEC["外部任务执行系统"]
        EXT_HOOK["外部服务 (Hooks通过curl调用)"]
    end

    subgraph "数据存储层"
        DB["SQLite数据库 (Workspace, Task, HookConfig, TaskQueue, Notification)"]
    end
...
3.  **可扩展性:**
    ...
    *   Hooks配置使用curl命令文本，灵活支持各种HTTP请求。
    *   使用TEXT类型存储JSON，预留扩展字段。
4.  **可追溯性:**
    *   所有关键操作（如任务创建、状态变更、队列执行）应记录操作日志，便于问题排查。
    *   任务失败原因、Hook执行错误等信息应详细记录，方便用户排查和重试。
5.  **性能优化:**
    *   对高频查询字段建立索引（已在表结构中定义）。
    *   SQLite适合单用户场景，数据量不宜过大（建议<100万条记录）。
    *   对于大量任务列表和队列，应采用分页加载。
    *   定时轮询频率应可配置，避免对外部服务造成过大压力。
...
- 主内容区域：核心展示区域，建议最小宽度720px
  - 页面头部：工作区名称、面包屑导航、项目目标摘要、以及"工作区设置"、"任务队列管理"、"添加任务"等操作按钮 - 建议高度60px
  - 工具栏区域：任务搜索框、筛选器（按状态、优先级）、批量操作（如添加到队列、批量删除） - 建议高度48px
...
**核心功能：**
展示特定工作区下的任务列表，提供任务的增删改查、优先级设置、状态管理；提供进入工作区设置和任务队列管理的入口。点击任务列表项可打开任务详情抽屉。
...
**页面布局架构：**
- 顶部导航栏：Logo、全局通知入口、用户头像/设置入口 - 建议高度60px，固定定位
- 左侧菜单：主导航菜单，突出显示当前工作区，可能包含子菜单如"基本设置"、"Hooks配置" - 建议宽度200-240px(展开)/48-56px(折叠)
- 主内容区域：核心展示区域，建议最小宽度720px
  - 页面头部：标题"工作区设置：[工作区名称]"，面包屑导航 - 建议高度48px
  - 内容展示区域：
    - **基本信息区:** 工作区名称、项目描述、项目目标编辑表单。
    - **Hooks配置区:** 列出已配置的Start/Stop Hooks，支持新增、编辑、删除Hook配置项，包括Hook名称、类型、curl命令文本、触发条件。
  - 底部：保存、取消按钮。
